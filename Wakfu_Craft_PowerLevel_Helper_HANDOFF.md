...
> **Qué es esto**: contexto completo para que otra instancia de Claude (u otra cuenta) retome este proyecto desde cero, sin haber visto la conversación original. Incluye el objetivo del proyecto, las instrucciones de comportamiento, el catálogo de fuentes que el usuario va a usar, lo que ya se sabe de los 5 archivos que subió, fórmulas de XP confirmadas, y una checklist de próximos pasos.
>
...
## 3. Preguntas pendientes para arrancar el trabajo real

En la próxima sesión, antes de calcular ninguna ruta, pedir al usuario (si no las ha dado ya en el mismo mensaje):
...
## 4. Catálogo de fuentes online aportadas por el usuario
...
- **craftkfu.waklab.fr** y **wakfujobcalculator.com** (incl. `/items-craft-guide`): son **SPAs (single-page apps) en JavaScript puro**; un `web_fetch` normal solo devuelve el esqueleto HTML vacío ("Craftkfu doesn't work properly without JavaScript"). **No se pueden leer sus datos mediante fetch/búsqueda desde el chat.** Para usarlas hace falta que el usuario las abra en su navegador y pegue aquí los resultados (texto o captura), o usarlas manualmente él mismo.
- **methodwakfu.com/artisanat/les-metiers/** — ✅ **fetch exitoso y con contenido muy valioso**, ver sección 9 (nueva) más abajo: resuelve varias de las dudas de la sección 7.
...
## 5. Archivos subidos por el usuario (ya analizados, resumen)

Los 5 archivos están en `/mnt/user-data/uploads/`. Si se pierden, pedir al usuario que los vuelva a subir. Resumen de cada uno:

### 5.1 `_Wakfu_Crafting_Professions_Leveling_Calculator__to_lvl_160__.xlsx`
...
### 5.2 `Wakfu_Sheets.xlsx` (comunidad, en inglés, multi-pestaña)
...
### 5.3 `Copia_de_New_tabla_de_oficios_wakfu.xlsx` (comunidad, en español)
...
- Tabla principal (tramos de 10 niveles, 0→160): Exp necesaria por tramo (igual patrón: 7500, 22500, 37500... +15000 cada tramo), Exp por componente (constante 660 en esta hoja — **contradice** los 300 EXP/craft "base" de la hoja 5.1; puede deberse a que ya incluye algún bonus, o a una versión distinta del juego — **hay que preguntar al usuario o contrastar**), cantidad de componentes necesarios, calidad del componente (Tosco→Ancestral, 17 tramos hasta 160).
...
### 5.4 `Tabla_de_oficios_de_fabricación__ES_.xlsx` (comunidad, en español)
...
- Es la hoja **más fácil de leer para sacar rápidamente "cuánto necesito de cada categoría de recurso" por tramo**, aunque no dice qué receta concreta craftear (para eso hacen falta las hojas 5.1/5.2/5.3 o las webs).

### 5.5 `_Wakfu__Récolte_métiers.docx` (comunidad, en francés — mecánicas de recolección/cultivo)
...
## 7. Datos verificados en fuente autorizada y reciente: methodwakfu.com/artisanat/les-metiers/ (última actualización 23-nov-2025, versión de juego citada: Wakfu 1.79)

Esta es, de las fuentes ya consultadas, **la más fiable y reciente**. Resuelve varias de las dudas de la sección 7. Todo lo de aquí abajo se puede dar por bueno salvo que el usuario aporte algo más nuevo.

### 9.1 Lista oficial de profesiones
...
### 9.2 Tabla de tramos de componente ↔ nivel de oficio ↔ nivel del ítem resultante
...
Esta tabla **coincide** con la columna "nivel de personaje" de la hoja 5.4 (`Tabla_de_oficios_de_fabricación (ES)`), lo cual la valida cruzadamente. Buena señal de que esa hoja sigue vigente en cuanto a estructura, aunque conviene revisar recursos concretos de los tramos altos (150+) por si hay parches más recientes.

### 9.3 Nivel máximo y componente base por profesión de fabricación
...
**Confirma y corrige la hoja 5.1** (`_Wakfu_Crafting_Professions_Leveling_Calculator`): Joyero y Armero comparten familia de recursos (minerales) como decía esa hoja, y el patrón de tramos de 10 en 10 hasta 170 es correcto, aunque el nivel máximo real del oficio es **170**, no 160 (la hoja subida se quedaba corta).

### 9.4 Fórmula de XP de crafteo (resuelve la contradicción de la sección 7.1 y 7.2)

> **XP obtenida = BaseXP × Additifs × BonusSiNivTropBas × BonusBP × BonusÉvènement**

Donde:
- **BaseXP**: XP base de esa receta concreta. **No es una constante universal** — cada receta tiene la suya. Esto explica por qué la hoja 5.1 usaba "300" como valor editable de ejemplo y la hoja 5.3 usaba "660": **ambas eran valores de ejemplo/placeholder para una receta concreta, no una constante del juego**. La forma correcta de obtener el BaseXP real de una receta es mirarlo en Craftkfu (que es justo la función que describe Jobkfu: "usa la base de XP de la receta indicada en Craftkfu").
- **BonusSiNivTropBas**: bonus si el jugador intenta la receta con menos del 100% de probabilidad de éxito (cuanto más por debajo del nivel recomendado, más XP extra si tiene éxito — pero also more risk of failure, ver 9.5).
...
- Bonos adicionales que se acumulan de forma aditiva entre sí (no multiplicativa): Guilde I +5%, Guilde II +5% (mejoras de gremio nivel 4 y 7), Havre-Monde +10% (una vez construido el edificio de oficio), Turbo Craft 30 +100% durante 30 min (crupier de compensación, cuesta 5 fichas), Mea Coulpable +100% durante 12h de juego (compensación antigua, no siempre disponible). **Ojo**: Turbo Craft 30, Potion d'Artisanat Véloce, Artipotion Exceptionnelle y Mea Coulpable **no son acumulables entre sí** (son excluyentes, hay que elegir uno).
  ...
  Esto **confirma** los inputs de la hoja 5.3 (`Copia_de_New_tabla_de_oficios_wakfu`): booster pack, bonus de gremio (%), evento de oficios (0/1) son exactamente estas variables.

- La **fórmula `75 + (N-1)×150`** del docx en francés (sección 5.5) **no aparece en esta fuente** para crafteo — es probable que sea una fórmula distinta (quizá de recolección/plantación en un contexto específico, o de una mecánica antigua). **No usar todavía para calcular XP de crafteo; tratar como no confirmada.**

### 9.5 Probabilidad de éxito y XP degresiva (mecánica nueva, no estaba en ninguna hoja subida)
...
- **Nivel máximo real del oficio = nivel de la última receta/recurso disponible + 20** (fórmula general del juego, consistente con la tabla de la sección 9.3: p.ej. si la última receta de fabricación es nivel 150, el oficio puede seguir subiendo craft eando esa receta hasta nivel 170 antes de quedarse sin poder progresar más, aunque ineficientemente).

### 9.6 Havre-Sac (Havenbag) y talleres — relevante para farmeo pasivo y crafteo cómodo
...
### 9.7 Herramientas comunitarias, confirmadas por la fuente oficial de referencia
...
## 8. Contradicciones / huecos detectados — estado actualizado (algunas ya resueltas en el milestone de la sección 7, NO inventar el resto)

1. ~~**EXP por componente**: 300 vs 660~~ → **RESUELTO** (ver 7.4): no es una constante universal, cada receta tiene su propio BaseXP; los valores 300/660 de las hojas 5.1/5.3 eran solo ejemplos editables. Para el BaseXP real de una receta concreta hay que consultarlo en Craftkfu.
2. **Fórmula de XP `75 + (N-1)×150`** del docx en francés (sección 5.5): sigue **sin verificar** — no aparece en la fórmula de fabricación confirmada por methodwakfu (sección 7.4). Hipótesis a confirmar con el usuario: podría ser de una mecánica distinta (recolección/plantación) o de una versión antigua del juego. **No usarla para calcular XP de crafteo.**
3. **Vigencia general de `Wakfu_Sheets.xlsx`** (hoja 5.2, en inglés): sigue sin confirmar su fecha real; puede tener años. La tabla de tramos de nivel de oficio de las hojas 5.1/5.3/5.4 **sí queda validada** por methodwakfu (coincide en tramos de "nivel de personaje"/"nivel de ítem"), pero los **recursos concretos de los tramos altos (150+)** de `Wakfu_Sheets.xlsx` siguen sin contrastar.
4. **Nivel máximo real de oficio**: **RESUELTO parcialmente** (ver 7.3): Armurier/Bijoutier/Maître d'Armes/Maroquinier/Tailleur = 170; Boulanger/Cuisinier = 160; Ébéniste = 165. Las hojas subidas que solo llegaban a 160-170 con tramos "???" quedan así explicadas: el "???" del tramo 160-170 en la hoja 5.4 corresponde justo a las profesiones que sí llegan a 170 (Armero, Joyero, Maestro de Armas, Marroquinero, Sastre) — pendiente aún conseguir los recursos concretos de ese último tramo.
5. ~~No se ha explorado el repo de GitHub ni hecho fetch en vivo~~ → **RESUELTO parcialmente** (ver sección 4): repo de GitHub revisado (es una app, no una fuente de datos); methodwakfu/les-metiers leído con éxito. **Sigue pendiente**: craftkfu y wakfujobcalculator no son accesibles por fetch (son SPAs JS) — necesitan que el usuario las use manualmente y pegue los resultados; y falta leer methodwakfu/localisation-des-ressources y methodwakfu/plans-et-recettes.
6. **Nuevo hueco detectado en este milestone**: no sabemos todavía el **BaseXP real** de ninguna receta concreta (solo la fórmula y de dónde sacarlo). Sin eso no se puede calcular "cuántos crafteos necesita el usuario" con precisión — es el dato bloqueante nº1 para la siguiente sesión.
7. **Nuevo dato a validar con el usuario**: la mecánica de "XP degresiva" (sección 7.5) implica que la estrategia óptima de ruta probablemente sea cambiar de receta bastante antes de los 20 niveles de diferencia (posiblemente hacia +10), lo que puede chocar con el objetivo de "pocas recetas distintas" del usuario (sección 1, punto 3) — hay un trade-off real entre "menos recetas" y "más XP por material" que hay que explicarle explícitamente cuando se calcule la ruta.

## 9. Plan de acción para la siguiente sesión (checklist)

Ya hechos en este milestone (no repetir):
- [x] Explorado el repo `wakfu-farm-tracker-public` → es una app de tracking, no una fuente de datos consultable (sección 4).
- [x] Confirmado que craftkfu.waklab.fr y wakfujobcalculator.com son SPAs JS no accesibles por fetch/búsqueda automática (sección 4).
- [x] Leído methodwakfu.com/artisanat/les-metiers/ en profundidad → resuelve niveles máximos por profesión, fórmula de XP de crafteo, probabilidad de éxito/XP degresiva, mecánica de Havre-Sac (sección 7).
- [x] Resuelta la contradicción "300 vs 660 XP por componente" (sección 8.1).

Pendientes reales para arrancar el trabajo con el usuario:
1. [ ] Preguntar al usuario los datos de la sección 3 (profesión, niveles, servidor, restricciones, bonuses activos).
...
3. [ ] Conseguir el **BaseXP real** de las recetas de la profesión elegida (bloqueante nº1, sección 8.6): pedir al usuario que abra Craftkfu para esa profesión y pegue las cifras, o pasar capturas de Jobkfu/wakfujobcalculator ya rellenados.
4. [ ] Con el BaseXP real, aplicar la fórmula completa de la sección 7.4 (con los bonuses activos del usuario) para calcular crafteos necesarios por tramo.
5. [ ] Decidir junto al usuario el trade-off de la sección 8.7 (menos recetas vs. más XP/material, por la XP degresiva a partir de +10/+20 niveles).
...
7. [ ] Proponer la ruta tramo a tramo con alternativas barata/segura, marcando explícitamente cualquier dato no confirmado (recursos de tramos 150-170 siguen sin contrastar en fuente reciente).
8. [ ] Si el usuario pide el plan como documento, generarlo en xlsx (reutilizando el formato de la hoja 5.4, que es el más claro) o en markdown, según prefiera.
